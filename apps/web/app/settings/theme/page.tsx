...
export default function ThemeSettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/theme`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then((response) => response.ok ? response.json() : Promise.reject(new Error(`Theme load failed (${response.status})`)))
      .then((theme) => theme?.tokens && setTokens(theme.tokens))
      .catch((error: unknown) => setMessage(error instanceof Error ? error.message : 'Theme load failed'));
  }, []);
  useEffect(() => {
  ...
  async function reason(response: Response) {
    ...
    try {
      const parsed = JSON.parse(body) as { error?: { message?: string | string[] }; message?: string | string[] };
  ...
  async function save() {
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'Theme save failed');
    }
  }
  async function reset() {
    try {
      ...
      if (response.ok) {
        const theme = await fetch(`${apiBase}/theme`, { headers: headers() }).then((result) => result.json());
        if (theme?.tokens) setTokens(theme.tokens);
      }
      setMessage(response.ok ? 'Defaults restored' : await reason(response));
    } catch (error: unknown) {
      setMessage(error instanceof Error ? error.message : 'Theme reset failed');
    }
  }
