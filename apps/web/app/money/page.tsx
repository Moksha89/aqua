...
export default function MoneyPage() {
  ...
  return <section>
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      <article className="rounded-xl border border-border bg-surface p-5"><h2 className="text-xl font-semibold">{t.payables}</h2>{(payables.data ?? []).map((row) => <div key={row.id} className="mt-3 flex justify-between border-b border-border pb-2 text-sm"><span>{row.expenseDate.slice(0, 10)}</span><span>{formatPaise(row.amountPaise)}</span></div>)}{payables.data?.length === 0 && <p className="mt-3 text-textSecondary">{t.noData}</p>}</article>
      <article className="rounded-xl border border-border bg-surface p-5"><h2 className="text-xl font-semibold">{t.receivables}</h2>{(receivables.data ?? []).map((row) => <div key={row.id} className="mt-3 flex justify-between border-b border-border pb-2 text-sm"><span>{row.dueDate?.slice(0, 10) ?? row.harvestDate.slice(0, 10)}</span><span>{formatPaise(row.receivablePaise)}</span></div>)}{receivables.data?.length === 0 && <p className="mt-3 text-textSecondary">{t.noData}</p>}</article>
    </div>
    <article className="mt-8 rounded-xl border border-border bg-surface p-5"><h2 className="text-xl font-semibold">{t.partyLedger}</h2><label className="mt-3 block max-w-md text-sm text-textSecondary">{t.party}<select value={selectedParty} onChange={(event) => setSelectedParty(event.target.value)} className="mt-1 w-full rounded-lg border border-border bg-background p-3 text-textPrimary"><option value="">—</option>{(parties.data ?? []).map((party) => <option key={party.id} value={party.id}>{party.name}</option>)}</select></label>{headroom.data && <div className="mt-4 grid gap-3 sm:grid-cols-3"><Metric label={t.creditLimit} value={headroom.data.limitPaise} /><Metric label={t.creditUsed} value={headroom.data.usedPaise} /><Metric label={t.creditHeadroom} value={headroom.data.headroomPaise} /></div>}{ledger.data?.entries.map((entry) => <div key={entry.id} className="mt-3 flex justify-between border-b border-border pb-2 text-sm"><span>{entry.kind} · {entry.date.slice(0, 10)}</span><span>{formatPaise(entry.amountPaise)}</span></div>)}</article>
