...
export default function HarvestPage() {
  ...
  async function completeClosure() {
    try {
      for (const step of ['CONFIRM_HARVESTS', 'ZERO_COST_HEADS', 'RECONCILE_FEED_STOCK', 'POST_OCCUPANCY_COSTS', 'CLOSURE_ALLOCATION']) await apiRequest(`/crops/${cropId}/closure-checklist/${step}`, { method: 'POST', body: JSON.stringify({ note: step === 'RECONCILE_FEED_STOCK' ? 'WRITE_OFF:No remaining stock' : undefined }) });
      const result = await apiRequest<CloseResponse>(`/crops/${cropId}/close`, { method: 'POST', body: JSON.stringify({}) });
      setClosed(result); setMessage(t.pnlFrozen);
    } catch (error) { setMessage(error instanceof Error ? error.message : t.closureFailed); }
  }
