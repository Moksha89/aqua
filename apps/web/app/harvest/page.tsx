...
export default function HarvestPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await apiRequest(`/crops/${cropId}/harvests`, { method: 'POST', body: JSON.stringify({ ...harvest, sampleTaken, sampleCount: sampleTaken ? harvest.sampleCount : undefined, sampleWeightG: sampleTaken ? harvest.sampleWeightG : undefined, lines: harvest.lines.map((line) => ({ ...line, ratePerKgPaise: rupeesToPaise(line.ratePerKgPaise) })) }) });
  ...
  return <section><h1 className="text-3xl font-semibold">{t.harvestClosure}</h1><p className="mt-2 text-textSecondary">{t.harvestGuidance}</p>{active?.status === 'CLOSED' ? <p className="mt-5 rounded-lg bg-warning/10 p-4 text-warning">{t.closedReadOnly}</p> : <form onSubmit={submit} className="mt-6 max-w-3xl rounded-xl border border-border bg-surface p-5"><div className="grid gap-3 sm:grid-cols-2"><Field label={t.harvestDate} type="date" value={harvest.harvestDate} onChange={(value) => setHarvest({ ...harvest, harvestDate: value })} /><ChoiceToggle label={t.harvestType} value={harvest.type} onChange={(value) => setHarvest({ ...harvest, type: value as Harvest['type'] })} options={[{ value: 'PARTIAL', label: t.partial }, { value: 'FINAL', label: t.final }]} /><ChoiceToggle label={t.pricingMethod} value={harvest.lines[0].basis} onChange={(value) => setHarvest({ ...harvest, lines: [{ ...harvest.lines[0], basis: value as 'COUNT' | 'GRADE' }] })} options={[{ value: 'COUNT', label: t.count }, { value: 'GRADE', label: t.grade }]} /><Field label={t.quantityKg} value={harvest.lines[0].quantityKg} onChange={(value) => setHarvest({ ...harvest, lines: [{ ...harvest.lines[0], quantityKg: value }] })} /><Field label={t.ratePaise.replace('paise/kg', '₹/kg').replace('పైసలు/కిలో', 'రూ./కిలో')} value={harvest.lines[0].ratePerKgPaise} onChange={(value) => setHarvest({ ...harvest, lines: [{ ...harvest.lines[0], ratePerKgPaise: value }] })} /></div><Disclosure label={t.moreDetails}><div className="mt-3 grid gap-3 sm:grid-cols-2"><Field label={t.doc} type="number" value={String(harvest.doc)} onChange={(value) => setHarvest({ ...harvest, doc: Number(value) })} /><SelectField label={t.harvestReason} value={harvest.reason} options={['TARGET_SIZE', 'MARKET_RATE', 'DISEASE', 'SEASON_END', 'OTHER'].map((value) => ({ value, label: optionLabel(value, 'en') }))} onChange={(value) => setHarvest({ ...harvest, reason: value as Harvest['reason'] })} /><Field label={t.lineKey} value={harvest.lines[0].key} onChange={(value) => setHarvest({ ...harvest, lines: [{ ...harvest.lines[0], key: value }] })} /><label className="flex gap-2 text-sm text-textSecondary"><input type="checkbox" checked={sampleTaken} onChange={(event) => { setSampleTaken(event.target.checked); setHarvest({ ...harvest, sampleTaken: event.target.checked }); }} />{t.sampleTaken}</label>{sampleTaken && <><Field label={t.sampleCount} type="number" value={String(harvest.sampleCount ?? '')} onChange={(value) => setHarvest({ ...harvest, sampleCount: Number(value) })} /><Field label={t.sampleWeight} value={harvest.sampleWeightG ?? ''} onChange={(value) => setHarvest({ ...harvest, sampleWeightG: value })} /></>}</div></Disclosure><button className="mt-5 rounded-lg bg-primary px-4 py-2 text-onPrimary" type="submit">{t.saveHarvest}</button></form>}{message && <p className="mt-4 rounded-lg bg-info/10 p-3 text-info">{message}</p>}<Disclosure label={t.readyToClose}><div className="mt-8 rounded-xl border border-border bg-surface p-5"><h2 className="text-xl font-semibold">{t.closureChecklist}</h2><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={startClosure} className="rounded-lg border border-border px-4 py-2">{t.startChecklist}</button><button type="button" onClick={() => completeClosure()} className="rounded-lg bg-primary px-4 py-2 text-onPrimary">{t.completeClosure}</button>{zeroCostHeads.length > 0 && <div className="mt-4 basis-full rounded-lg bg-warning/10 p-4 text-warning"><p>{t.zeroCostHeadsGuidance}</p><button type="button" onClick={() => completeClosure(zeroCostHeads)} className="mt-3 rounded-lg bg-warning px-4 py-2 text-onPrimary">{t.acknowledgeZeroCostHeads}</button></div>}</div>{closed?.pnlFrozen && <p className="mt-4 text-success">{t.pnlFrozen}</p>}</div></Disclosure></section>;
}
...
function optionLabel(value: string, language: 'en' | 'te'): string {
  const labels: Record<string, [string, string]> = {
    ...
    TARGET_SIZE: ['Right size', 'సరైన పరిమాణం'],
