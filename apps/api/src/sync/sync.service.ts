...
export class SyncService {
  ...
  async pull(since: string | undefined, limit: number, user: Context) {
    ...
    return { snapshot: snapshot.toISOString(), cursor: snapshot.toISOString(), hasMore: [crops, feedLogs, waterReadings, expenses].some((rows) => rows.length === limit), changes: { crops, feedLogs, waterReadings, ...(user.role === 'OPERATOR' && !user.financialAccess ? {} : { expenses }) }, theme: theme ? { tokens: theme.tokens, updatedAt: theme.updatedAt, revision: theme.rev } : null };
  }
  ...
  private async applyRecord(tx: Prisma.TransactionClient, record: SyncPushDto['records'][number], ctx: Context) {
    ...
    if (existing) {
      if (record.entity === 'feedLog') await tx.feedLog.update({ where: { id: record.id }, data: { ...record.payload as Prisma.FeedLogUpdateInput, rev: { increment: 1n }, updatedBy: ctx.userId } });
      else await tx.waterReading.update({ where: { id: record.id }, data: { ...record.payload as Prisma.WaterReadingUpdateInput, rev: { increment: 1n }, updatedBy: ctx.userId } });
      ...
      await tx.feedLog.create({ data: { ...(record.payload as Prisma.FeedLogCreateInput), id: record.id, businessId: ctx.businessId, createdBy: ctx.userId, updatedBy: ctx.userId, deviceId: ctx.deviceId } });
    } else if (record.entity === 'waterReading') {
      await tx.waterReading.create({ data: { ...(record.payload as Prisma.WaterReadingCreateInput), id: record.id, businessId: ctx.businessId, createdBy: ctx.userId, updatedBy: ctx.userId, deviceId: ctx.deviceId } });
    } else {
      await tx.expense.create({ data: { ...(record.payload as Prisma.ExpenseCreateInput), id: record.id, businessId: ctx.businessId, createdBy: ctx.userId, updatedBy: ctx.userId, deviceId: ctx.deviceId } });
    }
